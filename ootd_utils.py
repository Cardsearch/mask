...
# 填充图像中的空洞区域
def hole_fill(img):
    img = np.pad(img[1:-1, 1:-1], pad_width=1, mode='constant', constant_values=0)  # 填充图像的边缘
    img_copy = img.copy()
    mask = np.zeros((img.shape[0] + 2, img.shape[1] + 2), dtype=np.uint8)  # 创建一个遮罩
    # 假设 img 是一个 RGBA 图像（四通道）
    cv2.floodFill(img, mask, (0, 0), 255)  # 对空洞进行填充
    img_inverse = cv2.bitwise_not(img)  # 对图像取反
    dst = cv2.bitwise_or(img_copy, img_inverse)  # 合并图像
    return dst

# 对掩膜进行精细化处理，找到最大轮廓并填充
def refine_mask(mask):
    contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_TC89_L1)
    area = []
    for j in range(len(contours)):
        a_d = cv2.contourArea(contours[j], True)  # 计算轮廓面积
        area.append(abs(a_d))
    refine_mask = np.zeros_like(mask).astype(np.uint8)
    if len(area) != 0:
        i = area.index(max(area))  # 找到最大面积的轮廓
        cv2.drawContours(refine_mask, contours, i, color=255, thickness=-1)  # 填充最大轮廓

    return refine_mask

# 获取掩膜位置，基于模型解析结果、关键点和类别
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, width=384, height=512):
    im_parse = model_parse.resize((width, height), Image.NEAREST)  # 调整图像大小
    parse_array = np.array(im_parse)  # 转换为numpy数组
    
    if model_type == 'hd':
        arm_width = 60  # 设置手臂的宽度
    elif model_type == 'dc':
        arm_width = 45
    else:
        raise ValueError("model_type must be 'hd' or 'dc'!")  # 检查model_type是否合法
    ...
    pose_data = np.array(pose_data).reshape((-1, 2))  # 转换为numpy数组
    ...
    if category == 'dresses' or category == 'upper_body':
        shoulder_right = np.multiply(tuple(pose_data[2][:2]), height / 512.0)  # 计算肩膀右侧坐标
        shoulder_left = np.multiply(tuple(pose_data[5][:2]), height / 512.0)  # 计算肩膀左侧坐标
        elbow_right = np.multiply(tuple(pose_data[3][:2]), height / 512.0)  # 计算肘部右侧坐标
        elbow_left = np.multiply(tuple(pose_data[6][:2]), height / 512.0)  # 计算肘部左侧坐标
        wrist_right = np.multiply(tuple(pose_data[4][:2]), height / 512.0)  # 计算手腕右侧坐标
        wrist_left = np.multiply(tuple(pose_data[7][:2]), height / 512.0)  # 计算手腕左侧坐标
        ARM_LINE_WIDTH = int(arm_width / 512 * height)  # 设置手臂线条宽度
        ...
        if wrist_right[0] <= 1. and wrist_right[1] <= 1.:
            ...
            wrist_right = extend_arm_mask(wrist_right, elbow_right, 1.2)  # 扩展右手臂
        ...
        if wrist_left[0] <= 1. and wrist_left[1] <= 1.:
            ...
            wrist_left = extend_arm_mask(wrist_left, elbow_left, 1.2)  # 扩展左手臂
    ...
    return mask, mask_gray  # 返回最终掩膜和灰度掩膜
